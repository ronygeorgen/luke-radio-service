...
// Shared error parsing for settings API failures
const parseSettingsApiError = (error) => {
    ...
    if (apiError.buckets && Array.isArray(apiError.buckets)) {
      const bucketErrors = apiError.buckets
        .map((bucketError, index) => {
          ...
          return fieldErrors.length > 0 ? `Bucket ${index + 1}: ${fieldErrors.join('; ')}` : null;
        })
        .filter(Boolean);
      if (bucketErrors.length > 0) errorMessage = `Validation Error: ${bucketErrors.join('; ')}`;
...
/** Bulk update: one API request for all changed settings (and optional change_reason). */
export const updateSettingsBulk = createAsyncThunk(
  'settings/updateSettingsBulk',
  async ({ settingsUpdates, change_reason }, { getState, rejectWithValue, dispatch }) => {
    const { settings, settingsId, buckets, channelId } = getState().settings;
    if (!channelId) {
      return rejectWithValue('Channel ID is required');
    }
    if (!settingsUpdates || typeof settingsUpdates !== 'object' || Object.keys(settingsUpdates).length === 0) {
      return rejectWithValue('No settings to update');
    }

    const updatedFrontendSettings = { ...settings, ...settingsUpdates };
    ...
    try {
      await axiosInstance.post('/settings', payload);
      dispatch(fetchSettings(channelId));
      return { settingsUpdates };
    } catch (error) {
      const errorMessage = parseSettingsApiError(error);
      dispatch(fetchSettings(channelId));
...
const settingsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Update settings bulk (single API call for multiple changes)
      ...
      .addCase(updateSettingsBulk.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || action.error?.message || 'Failed to update settings';
      })
      // Add bucket
